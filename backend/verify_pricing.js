...
async function runTests() {
  ...
  try {
    ...
    // Seed DB to ensure collections are populated
    logger.info('[Test] Seeding database to initialize localized prices...');
    await seedDatabase();
    ...
    logger.info(`[Test] Indian Customer Starter -> Active Plan: ${subIN.plan_id}`);
    if (!subIN.plan_id.startsWith('plan_')) throw new Error('Razorpay Indian Plan ID mapping incorrect');
    ...
    logger.info(`[Test] US Customer Starter -> Active Plan: ${subUS.plan_id}`);
    if (!subUS.plan_id.startsWith('plan_')) throw new Error('Razorpay US Plan ID mapping incorrect');
