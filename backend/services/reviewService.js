...
async function processReviewWithSafety(outlet, review, providerOverride) {
  ...
  try {
    ...
    let aiResponse = null;
    let analysis = { issueCategory: 'General', emotion: 'Neutral' };

    try {
      analysis = await openaiService.analyzeReview({ rating, reviewText });
    } catch (analysisErr) {
      logger.warn('[ReviewService] Failed to analyze review category', { outletId: outlet.id, reviewId, error: analysisErr.message });
    }
