        ...
        <table className="w-full border-collapse text-left text-sm">
          <thead className="bg-slatey-50/80 text-xs font-medium uppercase tracking-wider text-slatey-500 dark:bg-slatey-900 dark:text-slatey-400">
            <tr>
              ...
              <th className="px-6 py-4">Subscription & Payment</th>
              <th className="px-6 py-4">AI Usage & Cost</th>
              <th className="px-6 py-4">WA Usage & Cost</th>
              <th className="px-6 py-4">Health / Risk</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm text-slatey-700 dark:text-slatey-300">{customer.contactPerson || customer.email || 'N/A'}</span>
                          <span className="text-[11px] text-slatey-400">{customer.phone || ''}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col items-start gap-1">
                          <StatusBadge status={customer.subscriptionStatus || (customer.accountStatus === 'Active' ? 'active' : 'inactive')} />
                          <div className="flex items-center gap-2">
                            <span className="text-[11px] text-slatey-600 font-semibold">{
                            ...
                            <span className="text-[11px] text-slatey-500">${
                              customer.plan === 'plan_starter' ? '29' :
                              customer.plan === 'plan_growth' ? '79' :
                              customer.plan === 'plan_premium' ? '199' :
                              customer.monthlyFee || 0
                            }/mo</span>
                          </div>
                          <span className="text-[10px] text-slatey-400">{customer.subscriptionStatus === 'trialing' ? 'Trial Active' : (customer.paymentStatus || 'Paid')} {customer.discountCode ? `(${customer.discountCode})` : ''}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-sm font-semibold text-slatey-700">{usage.aiResponses || 0} res</span>
                          <span className="text-xs text-brand-600">${aiCost}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-sm font-semibold text-slatey-700">{usage.whatsappAlerts || 0} alerts</span>
                          <span className="text-xs text-brand-600">${waCost}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col items-start gap-1">
                          <span className="text-[11px] bg-slatey-100 text-slatey-600 px-1.5 py-0.5 rounded">{healthScore}</span>
                          {customer.isChurnRisk && <span className="text-[10px] flex items-center gap-1 text-rose-500 font-semibold"><TriangleAlert className="h-3 w-3" /> Churn Risk</span>}
                          {customer.isUpsellTarget && <span className="text-[10px] flex items-center gap-1 text-emerald-500 font-semibold"><TrendingUp className="h-3 w-3" /> Upsell Target</span>}
                        </div>
                      </td>
