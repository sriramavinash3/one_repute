  ...
  const adminAccounts = useMemo(() => {
    return outlets.map(o => {
      ...
      return {
        ...
        aiCost: o.reviewCount ? o.reviewCount * 0.01 : 0, // Approx
        limit: o.aiUsageLimit || 100,
        status: o.isAutomationPaused ? 'Paused' : (o.isActive ? 'Active' : 'Inactive'),
        health: o.isAutomationPaused ? '85%' : '100%',
        original: o
      }
    })
  }, [outlets, customers])
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slatey-50/80 text-xs font-medium uppercase tracking-wider text-slatey-500">
              <tr>
                ...
                <th className="px-6 py-4">AI Usage (Cost / Limit)</th>
              ...
              {adminAccounts.map((acc) => (
                <tr key={acc.id} className="group hover:bg-slatey-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-semibold text-slatey-800">{acc.outletName}</p>
                    <p className="text-[10px] text-slatey-400">{acc.customerName}</p>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${acc.aiCost >= acc.limit ? 'text-rose-600' : 'text-slatey-700'}`}>${acc.aiCost.toFixed(2)}</span>
                      <span className="text-slatey-400">/ ${acc.limit.toFixed(2)}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                      acc.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : acc.status === 'Paused' ? 'bg-amber-100 text-amber-700' : 'bg-slatey-100 text-slatey-700'
                    }`}>
                      {acc.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-medium text-slatey-600 flex items-center gap-1.5">
                      {acc.health === '100%' || acc.health === '99.9%' ? <span className="w-2 h-2 rounded-full bg-emerald-500"></span> : <span className="w-2 h-2 rounded-full bg-amber-500"></span>}
                      {acc.health}
                    </span>
                  </td>
