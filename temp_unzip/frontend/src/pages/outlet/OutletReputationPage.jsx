...
import { fetchOutletReputationInsights } from '../../services/outletService'
...
export default function OutletReputationPage() {
  ...
  const { data: insights } = useQuery({
    queryKey: ['admin-reputation-insights'],
    queryFn: async () => {
      if (USE_MOCK_DATA) return MOCK_REPUTATION_INSIGHTS
      return await fetchOutletReputationInsights()
    }
  })
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slatey-50/80 text-xs font-medium uppercase tracking-wider text-slatey-500">
              <tr>
                ...
                <th className="px-6 py-4">Mentions (30d)</th>
